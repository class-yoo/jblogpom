...
public class FileUploadService {
	...
	// 파일업로드 위치는 밖으로 빼서 저장해야한다 .  /mysite-uploads 로시작
	private static final String SAVE_PATH = "/jblog/logo";
	// 프로젝트 경로 / images/ 를  경로로 설정 
	// spring-servlet.xml에서 설정하여 images/**의 파일을 찾는 요청이 오면 
	// 프로젝트의 가장최상단의 드라이드 현재는 d:/mysite-uploads/로 매핑되어 해당 폴더에서 파일을찾음  
	...
	public String restore(MultipartFile multipartFile) {
		...
		try {	
			...
			//  프로젝트가 실행된  곳의 최상위경로 d://mysite-uploads/파일명.확장자
